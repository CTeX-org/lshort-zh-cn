...
\begin{itemize}
  ...
  \hologo{TeXLive} 由类 UNIX 系统上的 te\TeX\ 发展并取而代之，最终成为跨平台的 \TeX\ 发行版。
  \hologo{TeXLive} 自 2011 年起以年份作为发行版的版本号，保持了一年一更的频率。

  Mac\TeX\ 是 macOS（OS X）系统下的一个定制化的 \hologo{TeXLive} 版本，与 \hologo{TeXLive} 同步更新。
...
\hologo{TeXLive} 和 \hologo{MiKTeX} 都集成了一个简单的 \LaTeX\ 源代码编辑器 \TeX works（Mac\TeX\ 则集成了类似的 \TeX shop）。
...
\hologo{TeXLive} 提供了图形界面的宏包管理器 \hologo{TeXLive} Manager 用于安装和更新宏包，而 \hologo{MiKTeX} 也提供了管理器
