...
TeXworks、TeXstudio、WinEdt 等提供了一些语法高亮、命令补全等功能，以及调用排版引擎完成编译的一些按钮。
一些流行的编辑器，或集成开发环境（IDE），如 vim、Emacs、Sublime Text、Eclipse 等，
也都提供了用于编写和编译 \LaTeX\ 源代码的插件和工具。
...
然后就可以用查看 PDF 的软件 （Adobe Reader / SumatraPDF 等）打开生成的 \texttt{hello\-world.pdf} 查看了。

\subsection{\texttt{pdflatex} 和 \texttt{xelatex} 命令}\label{subsec:pdflatex-exe}

\index{pdflatex@\texttt{pdflatex} 命令}
这两个命令相比于 \texttt{latex} 命令更为方便，我们可以直接输入以下命令编译源代码 \ref{code:hello-world}，生成 PDF：
\begin{verbatim}
