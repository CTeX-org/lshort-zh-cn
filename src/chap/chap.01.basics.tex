...
\begin{itemize}
  ...
  或者“排版”按钮。建议使用 “XeLaTeX” 作为默认的编译方式。
...
源代码~\ref{code:hello-world-chinese}~是在 \LaTeX\ 排版中文的一个最简示例。编译的方式与上一份源代码相同，使用 XeLaTeX 编译方式\footnote{注意两个问题：1. 文档保存为 UTF-8 编码；2. 某些发行版需要补充安装较多宏包（如 Mik\TeX\ ）。}。中文支持的详细内容见 \ref{sec:chinese} 节。
