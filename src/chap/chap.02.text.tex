...
本文的开头首先介绍了 \LaTeX\ 中排版中文的支持方式。随着 \LaTeX\ 和底层 \TeX\ 引擎的发展，旧方式（CCT、CJK 等）日渐退出舞台，
xelatex 编译命令配合 \cls{xeCJK} 或 \cls{ctex} 宏包的方式为当前的主流中文排版支持方式。
\end{intro}
...
由于 \TeX\ 最初设计用于排版以英文为主的西文文档，ASCII 编码完全够用，而早期版本的 \TeX\ 也只支持 7-bit 和 ASCII 编码。
...
使用 \texttt{latex} 及 \texttt{pdflatex} 编译命令时，这些编码的处理由 \pkg{inputenc} 宏包支持。
比如将源代码保存为 Latin-1 编码，并在导言区调用宏包：
\begin{verbatim}
\usepackage[latin1]{inputenc}
\end{verbatim}
接下来，M\"obius 就可以直接通过（用适当输入法）输入 \texttt{M\"obius} 得到了%
\footnote{并非所有拉丁文扩展字符都能够直接使用 \pkg{inputenc} 宏包排版出来。一些符号需要借助 \pkg{fontenc} 宏包对字体做进一步配置。}。

\index{bianma@编码!GBK}
GBK 等编码是多字节编码，ASCII 字符为一个字节，汉字等非 ASCII 字符为两个字节，使用 \texttt{latex} 及 \texttt{pdflatex} 编译命令时需要借助一些宏包进行较为复杂的判断和处理。
...
\pkgindex{xeCJK}
\texttt{xelatex} 命令支持直接使用操作系统和 \TeX\ 发行版安装的 TrueType (\texttt{.ttf}) / OpenType (\texttt{.otf}) 等格式的字体，加上对 UTF-8 编码的原生支持，
免去了预处理字体的麻烦。在此基础上的 \pkg{xeCJK} 宏包更进一步完善了排版中文的一些细节，比如中英文之间插入间距、
汉字之间的换行符去掉额外的空白、标点符号不出现在行首，等等。
