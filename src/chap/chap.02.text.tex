...
但位置靠顶端，（\cmd{\textasciitilde} 命令主要用作重音，参考下一小节）。
如果在 URL 中使用波浪号，应该使用 \pkg{url} 宏包的 \cs{url} 命令：
\begin{example}
\url{http://www.rich.edu/~bush}
\end{example}

在中文里表示数字范围的波浪号应该使用“～”（U+FF5E）：
\begin{example}
1～2
\end{example}
注意 \verb|$\sim$| 的用法是错误的。
另外，在西文里很少使用波浪号表示数字的范围
\footnote{\url{https://en.wikipedia.org/wiki/Tilde}}，
正确的方法是使用 en dash。
