...
\begin{description}
  ...
  \item[\cmd{mainmatter}] 正文部分，页码为阿拉伯数字格式，从 1 开始计数；其后的章节编号、页眉页脚和目录项正常。
  \item[\cmd{backmatter}] 后记部分，页码格式不变，继续正常计数；其后的章节不编号，但生成页眉页脚和目录项。
\end{description}
...
\end{titlepage}}
\end{verbatim}
事实上为标准文档类指定 \texttt{titlepage} 选项时，生成的标题页就是一个 \env{titlepage} 环境。
