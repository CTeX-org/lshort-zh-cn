...
$\CMcal{ABCDE}$                  & \cmd{mathcal}\marg*{\ldots}   & 近大写字母 \\
\hline
$\EuScript{ABCDE}$               & \cmd{mathcal}\marg*{\ldots}   & \pkg{eucal} 近大写字母 \\
$\mathscr{ABCDE}$                & \cmd{mathscr}\marg*{\ldots}   & \pkg{mathrsfs} 近大写字母\\
$\mathfrak{ABCDE abcde 1234}$    & \cmd{mathfrak}\marg*{\ldots}  & \pkg{amssymb} 或 \pkg{eufrak}  \\
$\mathbb{ABCDE}$                 & \cmd{mathbb}\marg*{\ldots}    & \pkg{amssymb} 近大写字母 \\
\hline
\end{tabular}
\end{table}
...
\footnote{国内使用粗斜体符号表示矢量，见 GB3102-11.93。}；再比如 \cmd{mathbf} 只能改变拉丁字母和大写希腊字母，小写希腊字母就没有用。
