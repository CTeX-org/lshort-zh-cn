...
让同一份 \hologo{BibTeX} 数据库生成不同样式的参考文献列表。我们首先简单介绍 \hologo{BibTeX} 数据库，之后介绍如何将数据库利用在 \LaTeX\ 中。
...
@article{Alice13,
  ...
  pages = {114-120}}
\end{verbatim}
所有类型的文献条目格式请参考 \CTAN|biblio/bibtex/base/btxdoc.pdf|。
...
\medskip
\textbf{IEEEtran}（\textsf{IEEEtran} 模板配套的样式）\\{}
...
第二步：在源代码中添加必要的命令。假设源代码的名称为 \texttt{demo.tex}（见 \ref{code:bibtex-demo}）。
\begin{enumerate}
